...
% FDWAVE_GEOMETRY_PLOT 
% The program to plot the source and reciever with and without model.
% Complete Syntax:
%          FDwave_geometry_plot('FIGNO',value,'HOP_S',value,'HOP_R',value)
...
% Example:
%       FDwave_geometry_plot('FIGNO',3,'HOP_S',1,'HOP_R',5);
%       FDwave_geometry_plot()
...
load([ipdirpath,'model'],'vpm','nh','nv','dh','dv')
load([ipdirpath,'geometry_rec'],'rnh_vec','rnv_vec');
load([ipdirpath,'geometry_src'],'snh_vec','snv_vec');
load([ipdirpath,'BC'],'BC')
...
% str=strcat(pwd,'\Data_OP\source_reciever_setup');
% saveas(gcf,str,'fig')
end
