function [src,t] = source_gaussian1(varargin)   %T,dt,f0,t0,src_scale)
% SOURCE_GAUSSIAN1
% This function generates the gaussian1 wavelet signature.
% Complete Syntax:
%       source_gaussian1('WFP',path,'T',value, 'DT',value, 'F0',value, 'T0',value, 'SRC_SCALE',value,'PlotON',option)
...
% Example:
%       source_gaussian1('WFP',pwd,'T',2,'DT',.0004,'T0',10)
%       source_gaussian1('T',2,'DT',.0004,'F0',10,'T0',.03,'SRC_SCALE',1,'PlotON','y');
...
disp([str0,'Source type                :    gaussian1 Wavelet'])
...
src = -src_scale*tau.*exp(-(alpha^2).*(tau.^2)/2);

% used for plotting the wavelet. td is time difference between side lobes of gaussian1.
tdn =round(3*t0/dt);  
src_name='Gaussian1 Wavelet';
