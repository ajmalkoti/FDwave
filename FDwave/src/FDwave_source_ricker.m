...
%       PlotON      : 'y'/'n' 
% Example:
%       source_ricker('WFP',pwd,'T',2,'DT',.0004,'F0',10)
...
if ~exist('T','var')      
    error('Provide total time duration, T, for simulation');    
end

if ~exist('dt','var')     
    error('Provide time step, dt, for simulation');             
end

if ~exist('f0','var')
    error('Provide source frequency, f0, for simulation ');     
end

str0='        ';   
str1=str0;          str2=str0;      

if ~exist('t0','var')||strcmp(num2str(t0),'-9999');    
    t0 = 1/(sqrt(2)*pi*f0);    str1=[str0,'(Default)'];    
end

if ~exist('src_scale','var')||strcmp(num2str(src_scale),'-9999');   
    src_scale=1;          
    str2=[str0,'(Default)'];   
end
